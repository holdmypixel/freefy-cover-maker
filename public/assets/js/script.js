...
const FREEFY_LOGO_WHITE_URL = 'https://freefy.app/storage/branding_media/5dc59df8-4008-49d1-9938-f93edd03cc0d.svg';
const FREEFY_LOGO_BLACK_URL = 'https://freefy.app/storage/branding_media/bba393e5-9ad9-482d-94f4-a07791346edf.svg';
...
function renderTextContent(ctx, canvas, textColor, theme) {
	...
	const useFreefyLogo = document.getElementById('useFreefyLogo').checked;
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	function setTheme(theme) {
		...
		render(); // Re-render canvas with new theme
	}
	...
	// Special handling for username field interaction with Freefy logo toggle
	const usernameField = document.getElementById('username');
	const freefyLogoToggle = document.getElementById('useFreefyLogo');
	const freefyLogoColorSelect = document.getElementById('freefyLogoColor');
	
	function updateUsernamePlaceholder() {
		if (usernameField && freefyLogoToggle) {
			if (freefyLogoToggle.checked) {
				usernameField.placeholder = "@username (leave empty for Freefy logo)";
			} else {
				usernameField.placeholder = "@username (or 'Music' if empty)";
			}
		}
	}
	
	function updateLogoColorVisibility() {
		if (freefyLogoColorSelect && freefyLogoToggle) {
			if (freefyLogoToggle.checked) {
				freefyLogoColorSelect.style.display = 'block';
			} else {
				freefyLogoColorSelect.style.display = 'none';
			}
		}
	}

	// Grain strength container visibility
	const grainCheckbox = document.getElementById('useGrain');
	const grainStrengthContainer = document.getElementById('grainStrengthContainer');
	
	function updateGrainStrengthVisibility() {
		if (grainStrengthContainer && grainCheckbox) {
			if (grainCheckbox.checked) {
				grainStrengthContainer.classList.remove('hidden');
			} else {
				grainStrengthContainer.classList.add('hidden');
			}
		}
	}

	if (grainCheckbox) {
		grainCheckbox.addEventListener('change', updateGrainStrengthVisibility);
		updateGrainStrengthVisibility(); // Initialize
	}
	
	if (usernameField && freefyLogoToggle) {
		// Update placeholder and logo color visibility when toggle changes
		freefyLogoToggle.addEventListener('change', () => {
			updateUsernamePlaceholder();
			updateLogoColorVisibility();
			render();
		});
		
		// Initialize placeholder and visibility
		updateUsernamePlaceholder();
		updateLogoColorVisibility();
	}
